...
class AutoManager:
    ...
    def load_homepage_urls(self) -> List[str]:
        """加载主页地址列表"""
        urls = []
        try:
            with open(self.homepage_urls_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        if line.startswith("https://www.douyin.com/user/"):
                            urls.append(line)
                        else:
    ...
    def save_processed_video(self, account_name: str, video_url: str):
        """保存指定账户已处理的视频链接"""
        try:
            processed_file = self.get_account_processed_file(account_name)
            # 如果文件不存在，先创建头部
    ...
    async def process_account_with_all_homepages(self, account: Dict[str, str], homepage_urls: List[str]) -> int:
        ...
        try:
            ...
            # 处理所有主页
            for homepage_idx, homepage_url in enumerate(homepage_urls, 1):
                print(f"\n  📍 处理主页 {homepage_idx}/{len(homepage_urls)}: {homepage_url}")
                ...
                try:
                    ...
                    newly_processed = await self._process_single_homepage(
                        page, account, homepage_url, processed_videos
                    )
                    
                    # 保存新处理的视频（按账户分别保存）
                    for video_url in newly_processed:
                        self.save_processed_video(account['name'], video_url)
    ...
    async def _process_single_homepage(self, page, account: Dict[str, str], homepage_url: str, processed_videos: Set[str]) -> List[str]:
        """处理单个主页（重用现有的页面实例）"""
        print(f"\n🔄 开始处理主页: {homepage_url}")
        ...
        try:
            ...
            (function() {
                ...
                // 方法1: 直接查找包含视频链接的li元素
                const liElements = document.querySelectorAll('li:has(a[href*="/video/"])');
                if (liElements.length > 0) {
                    console.log('使用方法1: li:has(a[href*="/video/"])，找到', liElements.length, '个元素');
                    liElements.forEach(item => {
                        const linkElement = item.querySelector('a[href*="/video/"]');
                        const imgElement = item.querySelector('img');
                        if (linkElement && imgElement) {
                            const href = linkElement.href;
                            const title = imgElement.alt || linkElement.title || '无标题';
                            videoItems.push({ title: title, link: href });
                        }
                    });
                } else {
                    // 方法2: 直接查找所有视频链接，然后找其父元素
                    console.log('方法1失败，尝试方法2: 直接查找视频链接');
                    const videoLinks = document.querySelectorAll('a[href*="/video/"]');
                    console.log('找到', videoLinks.length, '个视频链接');
                    
                    videoLinks.forEach(link => {
                        // 查找同级或父级的img元素
                        let imgElement = link.querySelector('img');
                        if (!imgElement) {
                            imgElement = link.parentElement?.querySelector('img');
                        }
                        if (!imgElement) {
                            imgElement = link.closest('li')?.querySelector('img');
                        }
                        
                        const href = link.href;
                        const title = imgElement?.alt || link.title || link.textContent?.trim() || '无标题';
                        videoItems.push({ title: title, link: href });
                    });
                }
                
                console.log('最终提取到', videoItems.length, '个视频');
                return videoItems;
            })();
            """
            ...
            for i, video_info in enumerate(new_videos):
                ...
                try:
                    ...
                    # 记录已处理的视频
                    newly_processed.append(video_url)
    ...
    async def run_auto_cycle(self, max_cycles: int = None):
        ...
        # 获取主页地址
        homepage_urls = self.load_homepage_urls()
        if not homepage_urls:
            print("❌ 没有可用的主页地址！")
            return
        
        print(f"📊 找到 {len(accounts)} 个账户，{len(homepage_urls)} 个主页地址")
        ...
        try:
            while True:
                ...
                for account_idx, account in enumerate(accounts, 1):
                    ...
                    print(f"📋 该账户将处理所有 {len(homepage_urls)} 个主页地址")
                    
                    try:
                        # 该账户处理所有主页地址
                        account_total_processed = await self.process_account_with_all_homepages(account, homepage_urls)
    ...
    while True:
        ...
        try:
            if choice == "1":
                ...
                homepage_urls = auto_manager.load_homepage_urls()
                ...
                for acc in accounts:
                    processed = auto_manager.load_processed_videos(acc['name'])
                    print(f"\n👤 账户: {acc['name']}")
                    print(f"  📹 已处理视频数: {len(processed)}")
                    
                    if processed:
                        recent_videos = list(processed)[-5:]  # 显示最近5个
                        print(f"  🕒 最近处理的视频:")
                        for i, video in enumerate(recent_videos, 1):
                            print(f"    {i}. {video}")
                        if len(processed) > 5:
                            print(f"    ... 还有 {len(processed) - 5} 个")
                    else:
                        print(f"  ℹ️ 暂无处理记录")
            
            elif choice == "10":
                # 手动清理Chromium进程
                before_count = auto_manager.get_chromium_process_count()
                print(f"\n🔍 清理前Chromium进程数: {before_count}")
                
                auto_manager.force_cleanup_chromium_processes()
                
                after_count = auto_manager.get_chromium_process_count()
                print(f"🔍 清理后Chromium进程数: {after_count}")
                
                if after_count > 0:
                    print("⚠️ 仍有进程未清理，可能需要重启系统")
                ...
                # 查看操作日志
                print(f"\n📋 操作日志:")
                for log_file in auto_manager.logs_dir.glob("*.log"):
                ...
                if not PYGAME_AVAILABLE:
                    print(f"\n🎵 音频功能未安装")
                    print("音频功能需要pygame库支持")
                    print("安装命令: pip install pygame")
                    print("\n安装完成后重启程序即可使用音频功能")
                else:
                    print(f"\n🎵 当前音频状态: {'🔊 开启' if auto_manager.audio_manager.enabled else '🔇 关闭'}")
                    print("1. 开启音频提醒")
                    print("2. 关闭音频提醒")
                    print("3. 测试音频")
                    ...
                    if audio_choice == "1":
                        auto_manager.audio_manager.enabled = True
                        print("✅ 音频提醒已开启")
                        # 播放测试音频
                        auto_manager.audio_manager.play_end_sound()
                    elif audio_choice == "2":
                        auto_manager.audio_manager.enabled = False
                        print("✅ 音频提醒已关闭")
                    elif audio_choice == "3":
                        print("🔊 测试音频播放...")
                        print("1. 任务完成音效")
                        ...
                        test_choice = input("请选择要测试的音效 (1-3): ").strip()
                        if test_choice == "1":
                            auto_manager.audio_manager.play_end_sound()
                        elif test_choice == "2":
                            auto_manager.audio_manager.play_phone_sound()
                        elif test_choice == "3":
